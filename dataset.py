...
class LmdbDataset(Dataset):
    ...
    def get_batch(self, ):
        choices = range(len(self))
        image_list, text_list = [], []
        while True:
            index = random.choice(choices)
            im, text = self.__getitem__(index)
            im = torch.unsqueeze(self.compose(im), 0)
            image_list.append(im)
            text_list.append(text)
            if len(text_list) == self.opt.batch_size:
                image_list = torch.cat(image_list, 0).float()
                return image_list, text_list

    def __getitem__(self, index):
        assert index <= len(self) or index == 0, 'index range error'
        ...
        with self.env.begin(write=False) as txn:
            ...
            if math.ceil(self.opt.imgH * ratio) >= self.opt.imgW:
                ...
                img = image.resize((self.opt.imgW, self.opt.imgH))
                return (img, label)
            else:
                resized_w = math.ceil(self.opt.imgH * ratio)

                resized_image = image.resize(
                    (resized_w, self.opt.imgH), Image.BICUBIC)

                new_PAD = Image.new(size=(self.opt.imgW, self.opt.imgH), color=(255),mode='L')
...
#     def __init__(self, max_size, PAD_type='right', PAD=1.0):
#         self.toTensor = transforms.ToTensor()
...
#         img.sub_(0.5).div_(0.5)
#         c, h, w = img.size()
#         Pad_img = torch.FloatTensor(*self.max_size).fill_(self.PAD)
